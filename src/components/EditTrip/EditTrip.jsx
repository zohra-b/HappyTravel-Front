...
export default function () {
    return (
        <div className="flex flex-col justify-center w-[750px] min-h-[487px] gap-6 rounded-2xl border-4 border-primary-yellow mt-48 mb-30 mx-auto px-14 py-6">

            <h2 className="text-xl text-tertiary-red font-bold pt-3 inline-block text-center">Editar destino</h2>
            <form className="border-t-2 border-tertiary-red flex justify-between mx-8">
                <div className="flex flex-col ">
                    ...
                    <div className={`relative ${styles.fileButton}`} >
                        <label for="custom-upload-btn" id="custom-upload-label" className={`absolute inset-0 flex items-center justify-center ${styles.customFileUpload}`}>
                            <img src=".\public\image\File-icon.svg" alt="Subir archivo" id="custom-upload-img" class="float-left w-6 h-6"/>
                        </label>
                        <input type="file" id="custom-upload-btn" name="image" accept="image/*" placeholder="Sube una imagen..." className={`pl-8 bg-primary-yellow rounded-2xl shadow-inner-top px-[1rem] placeholder:text-quaternary-blue py-[0.3rem] w-full text-[0.9rem] focus:outline-none ${styles.customFileUpload}`} />
                    </div>
